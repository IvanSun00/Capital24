...
<head>
    ...
    <title>Admin CAPITAL 2024 | Data Kelompok</title>
...
<body>
    ...
    <div class="container my-5">
        <h1 class="row justify-content-center" style="font-weight: 800">
            DATA KELOMPOK
        </h1>
                ...
                <div class="modal-body">
                    ...
                    <div class="row justify-content-center">
                        <div class="btn-group flex-wrap col-md-10" role="group" aria-label="Basic radio toggle button group">
                            ...
                            <input type="radio" data-val="empat" class="btn-check btn-anggota" name="btnradio" id="btnradio4" autocomplete="off">
                            <label class="btn btn-outline-dark" for="btnradio4">Keempat</label>
                        </div>
                    </div>
                    <hr>
    ...
    <!-- MODAL BUKTI UPLOAD -->
    <div id="imgmodal-2" class="modal">
        <span class="close">&times;</span>
        <img class="modal-contentt" id="img02">
    </div>
    </div>
    ...
    <script>
        $(document).ready(function() {
                                ...
                                $.ajax({
                                    ...
                                    success: function(data) {
                                        ...
                                        if (res[0] == "success") {
                                            ...
                                            Toast.fire({
                                                icon: 'success',
                                                title: 'Tim berhasil divalidasi!'
                                            });
                                        } else if (res[0].trim() == 'already') {
                                            getData();
                                            Toast.fire({
                                                icon: 'warning',
                                                title: 'Status Validasi Tim Sudah di Update!'
                                            });
                                        } else if (res[0] == "session") {
                                            document.location.href = res[1];
                                        } else if (res[0] == "error!") {
                                            Notiflix.Notify.Failure('Gagal<3');
                                        } else {
                                    ...
                                    $.ajax({
                                        ...
                                        success: function(data) {
                                            ...
                                            if (res[0].trim() == 'success') {
                                                ...
                                                Toast.fire({
                                                    icon: 'success',
                                                    title: 'Email kepada tim berhasil dikirim!'
                                                });
            ...
            $(document).on('click', '.btn-anggota', function() {
                ...
                if (urutan == 'satu') {
                    $("#cardAnggota").html(anggota1);
                } else if (urutan == 'dua') {
                    $("#cardAnggota").html(anggota2);
                } else if (urutan == 'tiga') {
                    $("#cardAnggota").html(anggota3);
                } else if (urutan == 'empat') {
                    $("#cardAnggota").html(anggota4);
                }
            });
            ...
            //FOTO MODAL BUKTI TRF
            $(document).on("click", ".bukti-tarif", function() {
                $('#img01').attr('src', "#");
                var path = '../uploads/buktitransfer/' + $(this).data("val");
                $('#img01').attr('src', path);
                var modal = document.getElementById("imgmodal-1");
                modal.style.display = "block";
                $('#imgmodal-1').modal('show');
            });
            ...
            $('.close').on('click', function() {
                $('#imgmodal-1').modal('hide');
            });
            //FOTO MODAL BUKTI UPLOAD
            $(document).on("click", ".bukti-upload", function() {
                $('#img02').attr('src', "#");
                var path = '../uploads/buktiupload/' + $(this).data("val");
                $('#img02').attr('src', path);
                var modal = document.getElementById("imgmodal-2");
                modal.style.display = "block";
                $('#imgmodal-2').modal('show');
            });
            $(document).on("click", ".img-custom", function() {
                $('#img02').attr('src', "#");
                var path = $(this).attr('src');
