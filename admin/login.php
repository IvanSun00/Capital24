...
$client->setRedirectUri('http://localhost/admin/login.php');
...
if (isset($_GET['code'])) {
    ...
    if(!isset($token['error'])){
        ...
        // check is admin
        $cek_admin_status = $conn->prepare("SELECT is_admin FROM `panitia` WHERE nrp = :nrp");
        $cek_admin_status->bindParam(':nrp', $nrp);
        $cek_admin_status->execute();

        $is_admin = $cek_admin_status->fetchcolumn();
        ...
        if (strlen($gmail) === 26 && substr($gmail, 9) === "@john.petra.ac.id" && $cek_email_nrp->rowCount() == 1 && $is_admin == 1) {
            $cek_email = true;
        }
...
<head>
    ...
    <title>Admin CAPITAL 2024 | Login</title> 
</head>
<body>
    <?php
    if (!empty($gagal))
